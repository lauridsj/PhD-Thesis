...
Thus, \chan is ideal for separating the \term{3}{P}{0} state from a mixed ensemble of \ttbar spin states, in the same way \chel is for the \term{1}{S}{0} state. Both \chel and \chan will be used in \cref{ch:ah} to search for new intermediate states in \ttbar production; the theory behind this is briefly discussed in \cref{sec:theory:ah}.
...
Besides the \mtt spectrum, one can infer the angular momentum state of the \ttbar bound state from the available production mechanisms at the LHC. 
...
Thus, the resulting \ttbar system must be in the \termc{1}{S}{0}{1} state. At NLO in QCD, also \termc{3}{S}{1}{1} states can be produced; however, the contribution is very small (less than $0.1\%$ of the total cross section~\cite{Kiyo:2008bv}).
...
Similarly, the observable \chan (\cref{eq:theory:chan}) has a maximal slope $D^{(k)}$ for the \term{3}{P}{0} state and is a good discriminator between H and the SM.
