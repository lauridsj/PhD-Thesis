...
\begin{figure}[!ht]
    ...
    \caption{\textbf{QCD control region for \mujets.} Distributions in the control region plots for data-driven QCD background estimation in Run-3 in the \mujets channel. From top left to bottom right: $\pt$ of the lepton, $\eta$ of the lepton, the number of jets, and the number of b-tagged jets. The uncertainty bands include MC statistical and systematic uncertainties. The difference between data and MC is considered QCD background. \todo{prettify plots}}
...
\begin{figure}[!ht]
    ...
    \caption{\textbf{QCD control region for \ejets.} Distributions in the control region plots for data-driven QCD background estimation in Run-3 in the \ejets channel, same as in \cref{fig:ttxs:np_plots_mj_CR}. \todo{prettify plots}}
...
Here, a data-driven normalization is derived for Z+jets events with one or two b tags in the dilepton channels, following \todo{EXO-22-014}. This is important especially in the same-flavor channels, where Z+jets is a dominant background.
...
This equation means that the \textit{ratio of ratios} $\Rinout( \geq \text{1 b tag} ) / \Rinout( \text{0 b tags} )$ is well described by MC.
It can be solved for the Z+jets yield outside of the Z windows in the same-flavor channels, yielding
...
Here, $f_b = \epsilon_b^{\mathrm{data}}/\epsilon_b^{\mathrm{MC}}$ is the unknown b tagging scale factor. It is left freely floating in the likelihood fit, 
