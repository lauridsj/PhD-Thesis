...
The resulting observed and expected limits for the combination of both channels are found in \cref{fig:ah:limit_1D_a_smtt_lx,fig:ah:limit_1D_h_smtt_lx,fig:ah:limit_1D_a_etat_lx,fig:ah:limit_1D_h_etat_lx}, again for both the ``A/H only'' and ``A/H + \etat'' scenarios. It can be seen that the large excess for low A/H masses is still present in the channel combination in the ``A/H only'' scenario, and is again stronger for the pseudoscalar A. The mild excess for the scalar H at $\mH \approx \SI{925}{\GeV}$, on the other hand, is not confirmed in the channel combination.
...
\begin{figure}[!ph]
    ...
    \caption{%
        Observed and expected exclusion limits on \gAtt in the combined dilepton and \ljets channels as a function of the mass of the A boson in the ``A only'' scenario. The expected limits in the dilepton and \ljets channels alone are shown as the red and purple lines for comparison. Otherwise, notations are equivalent to \cref{fig:ah:limit_1D_a_smtt}
    }
...
\begin{figure}[!ph]
    ...
    \caption{%
        Exclusion limits on \gHtt in the combined dilepton and \ljets channels as a function of the mass of the H boson in the ``H only'' scenario. Notations are equivalent to \cref{fig:ah:limit_1D_a_smtt_lx}.
    }
...
\begin{figure}[!ph]
    ...
    \caption{%
        Exclusion limits on \gAtt in the combined dilepton and \ljets channels as a function of the mass of the A boson in the ``A + \etat'' scenario. Notations are equivalent to \cref{fig:ah:limit_1D_a_smtt_lx}.
    }
...
\begin{figure}[!ph]
    ...
    \caption{%
        Exclusion limits on \gHtt in the combined dilepton and \ljets channels as a function of the mass of the H boson in the ``H + \etat'' scenario. Notations are equivalent to \cref{fig:ah:limit_1D_a_smtt_lx}.
    }
...
\begin{figure}[!t]
    ...
    \caption{%
        Frequentist 2D exclusion contours for \gAtt and \gHtt for four different signal hypotheses with identical A and H masses of \SI{365}{\GeV} (upper left), \SI{500}{\GeV} (upper right), \SI{750}{\GeV} (lower left) and \SI{1000}{\GeV} (lower right), all assuming a width of $2\%$. In all cases, \etat production is added to the background.
    }
...
\begin{figure}[!ph]
    ...
    \caption{%
        Frequentist 2D exclusion contours for \gAtt and \gHtt for six different signal hypotheses with differing A and H masses, corresponding to combinations of \SI{365}{\GeV}, \SI{500}{\GeV} and \SI{1000}{\GeV}, all assuming a width of $2\%$. In all cases, \etat production is added to the background.
    }
...
\section{Comparison to the ATLAS result}
